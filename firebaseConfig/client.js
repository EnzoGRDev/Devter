...
import { GithubAuthProvider, signInWithPopup, getAuth, onAuthStateChanged, getModularInstance  } from "firebase/auth"
...
function mapUserFromFirebaseAuthTo ({_tokenResponse, reloadUserInfo}){
  ...
  const { photoUrl, screenName } = _tokenResponse ? _tokenResponse : reloadUserInfo
...
export function handleOnAuthStateChanged (onChange){
  return onAuthStateChanged(authApp, (user)=>{
    const normalizedUser = mapUserFromFirebaseAuthTo(user)
